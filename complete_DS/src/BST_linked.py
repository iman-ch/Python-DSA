...
class BST:
    ...
    def __eq__(self, target):
        ...
        if self._count == 0 and target._count == 0:
            ...
            equals = self.__eq___aux(self._root, target._root)
        return equals

    def __eq___aux(self, sroot, troot):
        """
        ---------------------------------------------------------
        Determines whether two BSTs are equal.
        Values in self and target are compared and if all values are equal
        ...
        if sroot is None:
            ...
            if sroot._value == troot._value:
                equals = self.__eq___aux(sroot._right, troot._right)
                equals = self.__eq___aux(sroot._left, troot._left)
            else:
                equals = False
    ...
    def max_r_aux(self, curr):
        ...
        if curr._right is not None:
            self.max_r_aux(curr._right)
    ...
    def min_r_aux(self, curr):
        ...
        if curr._left is not None:
            value = self.min_r_aux(curr._left)
        return value
    ...
    def node_counts_aux(self, curr):
        ...
        if curr is None:
            ...
            left_zero, left_one, left_two = self.node_counts_aux(curr._left)
            right_zero, right_one, right_two = self.node_counts_aux(curr._right)
    
            if curr._left is None and curr._right is None:
                zero = 1
                one = 0
                two = 0
            elif curr._left is None or curr._right is None:
                zero = 0
                one = 1
                two = 0
            else:
    ...
    def total_depth(self):
        ...
        ---------------------------------------------------------
        """
        d = self.total_depth_aux(self._root, 0)
        
        return d
        

    def total_depth_aux(self, root, d):
        """
        ---------------------------------------------------------
        Returns the total depth of a bst.
        ---------------------------------------------------------
        ...
        if root is None:
            ...
            d += self.total_depth_aux(root._left, d+1) + self.total_depth_aux(root._right, d+1)
    ...
    def _mirror_aux(self, node):
        ...
        if node is not None:
            ...
            self._mirror_aux(node._left)
            self._mirror_aux(node._right)
    ...
    def _is_balanced_aux(self, node):
        ...
        if node is None:
            ...
            if abs(lh-rh) <= 1 and self._is_balanced_aux(node._left) is True and self._is_balanced_aux(node._right) is True: 
                balanced = True
    ...
    def retrieve_r(self, key):
        ...
        -------------------------------------------------------
        """
        value = self.retrieve_r_aux(key, self._root)
        return value

    def retrieve_r_aux(self, key, root):
        """
        -------------------------------------------------------
        Retrieves a _value in a BST. (Recursive)
        Use: v = bst.retrieve(key)
        ...
        if root is None:
            ...
            value = self.retrieve_r_aux(key, root._left)
        elif root._value < key:
            value = self.retrieve_r_aux(key, root._right)
        elif root._value == key:
            value = deepcopy(root._value)
    ...
    def average_depth(self):
        ...
        ---------------------------------------------------------
        """
        avg_depth = self.total_depth() // self._count
    ...
    def update(self, value, update):
        ...
        if curr is not None:
            curr._value = update(curr._value)
            updated = True
    ...
    def update_contains(self, value, update):
        """
        ---------------------------------------------------------
        Updates a value in a bst by applying a function to it.
        Use: bst.update(value, func)
        ...
        if not self.__contains__(value):
            ...
            #self.remove(curr._value)
            #self.insert(update(curr._value))
            curr._value = update(value)
    ...
    def update_r(self, key, update):
        ...
        --------------------------------------------------------- -
        """
        updated = self.update_r_aux(key, update, self._root)
        
        return updated

    def update_r_aux(self, key, update, root):
        """
        ---------------------------------------------------------
        Updates a value in a bst by applying a function to it.
        Use: bst.update(value, func)
        ...
        """
        updated = None
        if root is not None and root._value != key:
            if key < root._value:
                self.update_r_aux(key, update, root._left)
            elif key > root._value:
                self.update_r_aux(key, update, root._right)
                
        else:        
            if root is not None:
                updated = True
                root._value = update(root._value)
                root._update_height()
    ...
    def count_r(self):
        ...
        ----------------------------------------------------------
        """
        number = self.count_r_aux(self._root, 0)
        return number

    def count_r_aux(self, root, n):
        """
        ---------------------------------------------------------
        Returns the number of nodes in a BST.
        Use: number = bst.count()
        ...
        if root is None:
            ...
            n = 1 + self.count_r_aux(root._left, n) + self.count_r_aux(root._right, n)
