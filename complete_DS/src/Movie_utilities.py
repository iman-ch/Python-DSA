...
def read_movie(line):
    ...
    rating = float(rating)
    genres = genres.split(',')
    movie = Movie(title, year, director, rating, [int(g) for g in genres])
    return movie
...
def read_genres():
    ...
    """
    genres = []
    g_num = input("Enter a genre number (ENTER to quit): ")
    while g_num != "":
        digit = g_num.isdigit()
        if digit and int(g_num) > 10:
            print("Error: input must be <= 10")
        elif not digit or int(g_num) < 0:
            print("Error: not a positive number")
        elif int(g_num) in genres:
            print("Error: genre already chosen")
        else:
            genres.append(int(g_num))
        g_num = input("Enter a genre number (ENTER to quit): ")
...
def genre_counts(movies):
    ...
    -------------------------------------------------------
    """
    counts = [0 for i in range(len(Movie.GENRE_CODES))]
    for movie in movies:
